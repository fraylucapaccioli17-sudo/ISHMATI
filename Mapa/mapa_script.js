...
const municipios = [
  ...
  "emilianozapata", "axochiapan", "totolapan", "atlatlahucan", "huitzilac"
];
...
const preguntas = {
  ...
  tlayacapan: { texto: "¿Por qué es famoso Tlayacapan?", opciones: ["Sus exconventos", "Su cerámica", "Sus balnearios"], correcta: 0 },
  ...
  huitzilac: { texto: "¿Qué tipo de clima predomina?", opciones: ["Templado", "Frío", "Caluroso"], correcta: 0 }
};

// Función para municipios sin pregunta específica
function obtenerPregunta(municipio) {
  ...
  return {
    texto: `¿Qué caracteriza al municipio de ${nombreMunicipio} en Morelos?`,
    opciones: ["Su cultura y tradiciones", "Su gastronomía típica", "Su historia y arqueología"],
    correcta: 0
  };
}
...
function iniciarTemporizador() {
  ...
  reloj = setInterval(() => {
    ...
    if (tiempo <= 0) {
      clearInterval(reloj);
      document.getElementById("pregunta").innerHTML = `<div style="background:#ffcccc; padding:20px; border-radius:15px; text-align:center">
        <h3>⏰ ¡TIEMPO TERMINADO!</h3>
        <p>Has obtenido ${puntos} puntos.</p>
        <button onclick="reiniciar()" style="background:#0a2f6c; color:#fae47e; border:none; padding:10px 20px; border-radius:40px; margin-top:10px; cursor:pointer;">🔄 Jugar de nuevo</button>
      </div>`;
    }
  }, 1000);
}
